...
public sealed class RabbitMqJobEventPublisher : IJobEventPublisher, IAsyncDisposable
{
    ...
    public RabbitMqJobEventPublisher(
        IOptions<RabbitMqSettings> settings,
        ILogger<RabbitMqJobEventPublisher> logger)
    {
        _settings = settings.Value;
        _logger = logger;
    }

    public async Task PublishJobsFetchedAsync(JobsFetchedIntegrationEvent eventData, CancellationToken ct = default)
    {
        try
        {
            ...
            _logger.LogDebug(
                "Published JobsFetchedEvent {EventId} for source {SourceId}, {NewCount} new jobs",
                eventData.EventId, eventData.JobSourceId, eventData.NewJobCount);
        }
        catch (Exception ex)
        {
    ...
    private async Task EnsureInitializedAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            _logger.LogInformation(
                "Jobs service connected to RabbitMQ at {Host}:{Port}",
